...
When interaction terms are added, \texttt{logitmfx} reports the partial derivative of the predicted probability with respect to each variable, \emph{including} the contribution through the interaction term evaluated at the sample means. For variables not in any interaction, effects are virtually unchanged. For interacted variables, the reported marginal effect now reflects the \emph{conditional} effect at the mean of the interacting partner. Specifically:
\begin{itemize}
  \item \texttt{duration\_days} flips sign because the marginal effect is now $\frac{\partial P}{\partial \text{duration}} \propto (\beta_{\text{duration}} + \beta_{\text{interaction}} \times \overline{\text{log\_size}})$, and evaluated at mean log-size this is negative---but this is misleading since for any realistic protest ($>75$ people) the net effect remains positive (see Section on coefficient signs).
  \item \texttt{demand\_police} increases because its marginal effect now captures the ``pure'' demand premium at mean cum\_state\_violence, which is higher than the average effect pooled over all repression levels.
  \item \texttt{cum\_state\_violence} increases for the same reason---it now captures the repression effect at mean demand\_police.
\end{itemize}
...
\paragraph{Duration ($-$0.2pp, $p = 0.001$; was +0.1pp before, $p = 0.076$).}
The marginal effect of one additional day flips from $+0.1$pp (marginally significant) to $-0.2$pp (significant). \emph{Why the sign flips}: This is a mechanical artefact of the interaction decomposition---the reported marginal effect is now $\frac{\partial P}{\partial \text{duration}} = f'(\cdot) \times (\beta_{\text{dur}} + \beta_{\text{int}} \times \overline{\text{log\_size}})$, evaluated at the sample mean. Because the main effect of duration is negative in the presence of the interaction (it represents the effect at log\_size $= 0$), the reported marginal effect at the mean can appear negative depending on the evaluation point. \emph{Substantive interpretation}: As shown in the coefficient-signs section, for any realistically sized protest ($>75$ people), longer duration \emph{still increases} violence probability---consistent with the economic intuition that prolonged protests deplete participants' patience, strain policing resources, and create repeated opportunities for provocation incidents.
...
\caption{ROC curve of the final logit model. The area under the curve (AUC) summarises the model's ability to rank violent above peaceful protests across all classification thresholds.}
\label{fig:roc}
\end{figure}
